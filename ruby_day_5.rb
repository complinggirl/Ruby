...
#
# def longWord (word)
#   $maxLen = word if word.length > $maxLen.length
# end
#
# snowy_words.each {|word| longWord(word)}
# puts $maxLen
...
new_snowy = []
snowy_words.each{|word| new_snowy.push(word) if not dictionary.include?(word)}
new_clancy = []
clancy_words.each{|word| new_clancy.push(word) if not dictionary.include?(word)}
puts (new_snowy + new_clancy).uniq.join ', '


# dico = Array.new File.read('/usr/share/dict/words').lines.map{|w| w.chomp}
# puts dico.length
#
# new_snowy = []
# snowy_words.each{|word| new_snowy.push(word) if not dictionary.include?(word)}
